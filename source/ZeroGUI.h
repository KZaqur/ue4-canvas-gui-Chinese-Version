...
// 添加 crypt 函数定义
const char* crypt(const char* str) {
	return str;
}
...
static std::wstring utf8_to_wstring(const char* utf8)
{
	...
	int lenRequired = MultiByteToWideChar(CP_UTF8, 0, utf8, -1, NULL, 0);
	if (lenRequired == 0) return std::wstring();

	std::wstring w;
	w.resize(lenRequired - 1);
	MultiByteToWideChar(CP_UTF8, 0, utf8, -1, &w[0], lenRequired);
...
namespace ZeroGUI
{
	...
	{
		// 把 UTF-8 字符串转成宽字符
		std::wstring w = utf8_to_wstring(name);
		// FString 的第二/三参数通常是字符数（根据你的 SDK 定义），这里传入宽字符长度+1 会更安全
		int wchar_len_plus_null = static_cast<int>(w.size()) + 1;
		canvas->K2_DrawText(MediumFont, FString{ const_cast<wchar_t*>(w.c_str()), wchar_len_plus_null, wchar_len_plus_null }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	...
	void TextCenter(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		std::wstring w = utf8_to_wstring(name);
		int wchar_len_plus_null = static_cast<int>(w.size()) + 1;
		canvas->K2_DrawText(MediumFont, FString{ const_cast<wchar_t*>(w.c_str()), wchar_len_plus_null, wchar_len_plus_null }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
	...
	void SliderFloat(char* name, float* value, float min, float max, const char* format = "%.0f")
	{
		elements_count++;
	...
	void Hotkey(char* name, FVector2D size, int* key)
	{
		...
		if (active_hotkey == elements_count)
		{
			//Text
			FVector2D textPos = FVector2D{ pos.X + size.X / 2, pos.Y + size.Y / 2 };
			TextCenter(const_cast<char*>(crypt("[Press Key]")), textPos, FLinearColor{ 1,1,1,1 }, false);
